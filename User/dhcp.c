...
uint8 check_DHCP_state(SOCKET s) 
{
	...
	type = 0;
	if(uchFN_W5500getSn_SR(s)!=SOCK_CLOSED)                        /*socket处于打开状态*/
	{
		if ((len = unFN_W5500getSn_RX_RSR(s)) > 0)									/*接收到数据*/
		{
...
void init_dhcp_client(void)
{
	...
	vFN_iinchip_init();
	vFN_W5500setSUBR(ip_0);
	vFN_W5500setGAR(ip_0);
	vFN_W5500setSIPR(ip_0);
