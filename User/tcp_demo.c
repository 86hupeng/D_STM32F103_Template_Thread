...
{	
	uint16 len=0;  
	switch(uchFN_W5500getSn_SR(SOCK_TCPS))											            	/*获取socket的状态*/
	{
		...
		case SOCK_ESTABLISHED:												              /*socket处于连接建立状态*/
			...
			{
				vFN_W5500setSn_IR(SOCK_TCPS, Sn_IR_CON);								          /*清除接收中断标志位*/
			}
			len=unFN_W5500getSn_RX_RSR(SOCK_TCPS);									            /*定义len为已接收数据的长度*/
			if(len>0)
			{
...
{	
   uint16 len=0;	
	...
	switch(uchFN_W5500getSn_SR(SOCK_TCPC))								  				         /*获取socket的状态*/
	{
		case SOCK_CLOSED:											        		         /*socket处于关闭状态*/
		...
		case SOCK_ESTABLISHED: 												             /*socket处于连接建立状态*/
			...
			{
				vFN_W5500setSn_IR(SOCK_TCPC, Sn_IR_CON); 							         /*清除接收中断标志位*/
			}
		
			len=unFN_W5500getSn_RX_RSR(SOCK_TCPC); 								  	         /*定义len为已接收数据的长度*/
			if(len>0)
			{
